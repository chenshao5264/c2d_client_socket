...
void TCPSocket::send(char * pData, size_t uLen)
{
    ...
    {
        this->doWrite(pData, uLen);
    }
}

void TCPSocket::send(Pack & pack)
{
    this->send(pack.getData(), pack.getLenght());
}
...
bool TCPSocket::doRead()
{
    ...
    else 
    {
        if (m_pDelegate)
        {
            ...
            m_pDelegate->onMessage(m_pReadBuffer);
        }
    }
    return true;
}

bool TCPSocket::doWrite(char* pData, size_t uLen)
{
    // 打包（序列化）
    ...
    int nRet = m_pSocket->write(pData, uLen);
